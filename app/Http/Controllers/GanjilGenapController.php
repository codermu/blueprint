...
define('BOT_TOKEN', '242067186:AAGqYGV9TmfYXTzCjzU-LXRehpEw5Owlxgs');
define('API_URL', 'https://api.telegram.org/bot'.BOT_TOKEN.'/');
define('WEBHOOK_URL', 'https://festiware.com/ganjilgenap');
...
class GanjilGenapController extends Controller
{
	...
	public function main(Request $request) {
		if ($request->input('set')==1) {
		  ...
		  $status = $this->apiRequest('setWebhook', array('url' => isset($argv[1]) && $argv[1] == 'delete' ? '' : WEBHOOK_URL));
		  exit;
		}
		
		$content = file_get_contents("php://input");

		Storage::disk('local')->put('debug.txt', $content);
		
		$update = json_decode($content, true);
		
		if (!$update) {
		  // receive wrong update, must not happen
		  Storage::disk('local')->put('debug.txt', "ada update");
